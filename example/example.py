...
# tracer_mge = table.QTable.read("mge.dat", format="ascii", names=["n", "i", "s", "q"])
# tracer_mge["i"].unit = 1 / u.pc**2
# tracer_mge["s"].unit = u.arcsec


tracer_mge = table.QTable.read("../../omegaCen/mgefit/mge.esv", format="ascii.ecsv")

tracer_mge["i"] = tracer_mge["I"]
tracer_mge["s"] = tracer_mge["sigma"]

tracer_mge["i"].unit = 1 / u.pc**2
tracer_mge["s"].unit = u.arcmin

del tracer_mge["I"]
del tracer_mge["sigma"]
...
kin = table.QTable.read("../../omegaCen/rotation/kinematics.escv", format="ascii.ecsv")
...
moments["sigma_x"] = np.sqrt(np.abs(moments["v2xx"] - moments["vx"] ** 2))
moments["sigma_y"] = np.sqrt(np.abs(moments["v2yy"] - moments["vy"] ** 2))
moments["sigma_z"] = np.sqrt(np.abs(moments["v2zz"] - moments["vz"] ** 2))
...
plt.figure()
plt.title("No Rotation, sigma_z")
plot_velfield(x=kin["x"].value, y=kin["y"].value, vel=moments["sigma_z"].value, colorbar=True)
plt.show()
...
# set dispersions from first and second moments
moments_rot["sigma_x"] = np.sqrt(np.abs(moments_rot["v2xx"] - moments_rot["vx"] ** 2))
moments_rot["sigma_y"] = np.sqrt(np.abs(moments_rot["v2yy"] - moments_rot["vy"] ** 2))
moments_rot["sigma_z"] = np.sqrt(np.abs(moments_rot["v2zz"] - moments_rot["vz"] ** 2))

# write moments out to file
table.Table(moments_rot).write("new_moments.dat", format="ascii.ecsv", overwrite=True)

# read in the example moments file
example = table.QTable.read(
    "moments.dat", format="ascii", names=["vx", "vy", "vz", "v2xx", "v2yy", "v2zz", "v2xy", "v2xz", "v2yz"]
)
...
example["sigma_z"] = np.sqrt(example["v2zz"] - example["vz"] ** 2)
...
plt.figure()
plot_velfield(x=kin["x"].value, y=kin["y"].value, vel=moments_rot["vz"].value, colorbar=True)
plt.show()

plt.figure()
plot_velfield(x=kin["x"].value, y=kin["y"].value, vel=moments_rot["sigma_z"].value, colorbar=True)
plt.show()
